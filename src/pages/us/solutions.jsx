...
export default function Solutions(props) {
  ...
  const getTargetedSolutions = () => {
    ...
    const contactOptionsLocalized = usContactOptions;

// 根据用户选择的催收方式生成{'Targeted plan'}
  ...
  const handleCopy = (text, id) => {
    ...
    toast({
      ...
      description: 'Text copied — you can paste and use it.'
    });
  ...
  const negotiationScripts = [{
    ...
    id: 2,
    title: 'Request a temporary hardship extension',
    category: 'Negotiation scripts',
    ...
    id: 3,
    title: 'Request interest/fee reduction',
    category: 'Negotiation scripts',
    ...
    id: 5,
    title: 'Handling a collection call',
    ...
    id: 6,
    title: 'Decline unreasonable demands',
    category: 'Response scripts',
  ...
  const checklists = [{
    ...
    items: ['Stay calm and respectful', 'Be honest—don’t exaggerate or hide key facts', "Don’t promise what you can’t deliver", 'Write down what was agreed', 'Ask for written confirmation']
  }, {
    id: 3,
    title: 'After negotiation',
    items: ['Confirm the outcome and keep written proof', 'Follow the agreed plan on time', 'Check in periodically on progress', 'Contact them quickly if issues arise', 'Keep all payment records']
  }];
  const communicationTips = [{
    id: 1,
    title: 'Before communicating',
    tips: ['Stay calm—avoid emotional reactions', 'Prepare documents and evidence', 'Know your budget and boundaries', 'Pick a good time to talk', 'Make sure the environment is quiet']
  }, {
    id: 2,
    title: 'Key points while communicating',
  ...
  const complaintInfo = [{
    ...
    id: 2,
    title: 'Things you can complain about',
    type: 'Trigger-based',
      ...
      <header className="bg-[#1E3A5F] text-white py-3 md:py-4 px-4 md:px-8 shadow-lg">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <span onClick={() => navigateTo({ pageId: 'us/home' })} role="button" tabIndex={0} className="text-lg md:text-xl font-bold font-['Space_Grotesk'] hover:opacity-80 cursor-pointer">{'Solutions'}</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              ...
              <p className="text-xs md:text-sm text-[#64748B]">
                {riskLevel === 'low' && 'Low risk — stay documented and pay on schedule if possible'}
                {riskLevel === 'medium' && 'Medium risk — negotiate proactively and set a plan'}
                {riskLevel === 'high' && 'High risk — consider professional help and proceed carefully'}
              </p>
            </div>
            ...
            <div className="space-y-4 md:space-y-6">
              {targetedSolutions.map(solution => <Card key={solution.id} className={`${solution.illegal ? 'bg-gradient-to-br from-red-50 to-orange-50 border-2 border-red-200' : 'bg-white'} rounded-xl p-4 md:p-6 shadow-lg hover:shadow-xl transition-shadow`}>
                  ...
                  {solution.illegal && solution.law && <div className="mb-3 md:mb-4 p-3 md:p-4 bg-red-100 border-l-4 border-red-500 rounded-r-lg">
                      <p className="text-xs font-semibold text-red-700 mb-1">{'Reference:'}</p>
                  ...
                  <div className="mb-3 md:mb-4">
                    <p className="text-xs font-semibold text-[#1E3A5F] mb-2">{'What to do:'}</p>
            ...
            <div className="space-y-4 md:space-y-6">
              {complaintInfo.map(info => <Card key={info.id} className="bg-white rounded-xl p-4 md:p-6 shadow-lg">
                  <div className="flex items-center gap-2 md:gap-3 mb-3 md:mb-4">
                    <div className={`w-8 h-8 md:w-10 md:h-10 rounded-lg flex items-center justify-center ${info.type === 'Trigger-based' ? 'bg-red-100' : 'bg-blue-100'}`}>
                      <FileText className={`w-4 h-4 md:w-5 md:h-5 ${info.type === 'Trigger-based' ? 'text-red-600' : 'text-blue-600'}`} />
                    </div>
                    <div>
                      <h3 className="text-base md:text-lg font-bold font-['Space_Grotesk'] text-[#1E3A5F]">
                        {info.title}
                      </h3>
                      <span className={`text-xs font-semibold ${info.type === 'Trigger-based' ? 'text-red-600' : 'text-blue-600'}`}>
                        {info.type}
                      </span>
                    </div>
                  </div>
                  <div className="space-y-2 md:space-y-3">
                    {info.content.map((item, index) => <div key={index} className={`p-3 md:p-4 rounded-lg ${info.type === 'Trigger-based' ? 'bg-red-50 border-l-4 border-red-500' : 'bg-blue-50 border-l-4 border-blue-500'}`}>
